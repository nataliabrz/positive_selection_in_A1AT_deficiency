...
LCM_calls_path <- "outputs/combined_collapsed_dnds_input.tsv"

outputPath <- "outputs/"

## load dnds object to annotate muts and colour by impact
annotmuts <- fread("outputs/A1AD_dndsout_annotmuts_PDv38is_wgs.tsv", sep="\t")
sel_cv <- fread("outputs/A1AD_dndsout_selcv_PDv38is_wgs.tsv", sep="\t")
...
dndsPath <- paste0(homedir,"/analysis/dnds/NAFLD_LCM/run_2_with_mendeley_data/")
LCM_calls_path <- paste0(homedir,"/analysis/dnds/NAFLD_LCM/run_2_with_mendeley_data/combined_collapsed_dnds_input.tsv")

}else if(cohort=="Obese"){

dndsPath <- paste0(homedir,"/analysis/dnds/Amsbio_lcm/")
LCM_calls_path <- paste0(homedir,"/analysis/dnds/Amsbio_lcm/unique_dinuc_collapsed_5col.tsv")
...
## merge lcm_data with dnds annotations and clinvar annotations
lcm_data <- lcm_data |> 
...
#drivers <- dndsout$sel_cv[which(dndsout$sel_cv$qglobal_cv < 0.1 | dndsout$sel_cv$qmis_cv<0.01),"gene_name"]

## define genes of interest by dNdS sig genes or by custom list or put in gene names (example below)
#goiFull <- read_tsv("/Users/mn7/volumes/mn7_lustre/sperm/targeted/dnds/sigGenes.tsv", col_types = cols()) |> distinct(gene_name) |>  pull(gene_name)
# goi <- c("CUL3", "RASA2", "ARID1A", "NF1", "RIT1","KRAS", "PTPN11", "HRAS", "BRAF", "DDX3X", "FGFR3", "CBL", "NSD1", "BMPR2", "EP300", "SMAD4", "FGFR2")


if(cohort %in%  c("SLD","Obese")){
goiInterest <- c("FOXO1","CHD4","GPAM","CIDEB","ACVR2A","SERPINA1","HFE","INSR","FASN","A1CF","TNRC6B","CDKN1B","CYP2E1","ALB","TF","UBE2D3","HPN","SF3B3","EEF2")
} else if(cohort %in% c("A1AD","HH")){
goiInterest <- c("SERPINA1","HFE","ACVR2A")
}
...
geneDomains <- geneDomainsCollect |> 
  ...
  dplyr::select(-c(evidences, molecule, ligand, begin, end)) 
  
custom_domains_df <- data.frame(
  gene = c("SERPINA1", "HFE", "CHD4", "CHD4", "CHD4", "CHD4", "CHD4"),
  description = c("RCL", "MHC I-like", "C1a", "C1b", "C2a", "C2b", "HMG"),
  recStart = c(1095, 81, 3700, 4200,5100, 5505, 450),
  recEnd = c(1158, 600, 3930, 4620, 5495, 5730, 645)
)
...
gene <- gene0 |> 
  ...
  arrange(cdsPos) |> 
  drop_na(cdsPos) |>
  #mutate(name = ifelse(impact == "Indel", paste0(ceiling(cdsPos/3), name), name)) |>
  rowwise() 
...
if ( goi[g] == "SERPINA1") {
write.table(gene,"outputs/SERPINA1_annotmuts.tsv",sep="\t",col.names=T,row.names=F,quote=F)
}
  ...
  pCov <- ggplot(variants, aes(x = cdsPos, y = count)) + 
    #Coverage area
    #geom_area(data = geneCov |> mutate(count = cov/(200000/yMaxCount)), fill = '#ccece6') +
    #Variant stems
  ...
  title = goi[g],
  subtitle = if(qval == 0) {
    paste0(cohort," cohort (n=", length(unique(str_extract(lcm_data$sampleID, "PD\\d+"))), "), q value < ", expression(2 %*% 10^16))
  } else {
  ...
  pCov <- ggplot(variants, aes(x = cdsPos, y = count)) + 
    #Coverage area
    #geom_area(data = geneCov |> mutate(count = cov/(maxCov/yMaxCount)), fill = '#ccece6') +
    #Variant stems
    geom_segment(data = variants, aes(x=cdsPos, xend=cdsPos, y=0, yend= (0.2 * count))) +
   #ggtitle(goi[g]) +
    #Label clinvar
  ...
  filename <- paste0(outputPath, goi[g],"_lcm_lollipop_",cohort,"_cohort.pdf")  
  }else{
  filename <- paste0(outputPath, goi[g],"_noncoding_unfiltered_lcm_lollipop_",cohort,"_cohort.pdf")
  }
