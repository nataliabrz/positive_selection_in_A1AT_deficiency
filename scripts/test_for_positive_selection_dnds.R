...
wgs_calls <- fread("data/calls/wgs_calls.csv", sep = ",") %>%
    dplyr::select(sampleID, Chrom, Pos, Ref, Alt, Gene)
...
close_snvs <- snv_calls[
  order(sampleID, Chrom, Pos), # Ensure sorting
  .(sampleID, Chrom, Pos, next_pos = shift(Pos, type = "lead")),
  by = .(sampleID, Chrom)
][
...
# Display results
close_snvs[
  , .(sampleID, Chrom, Pos, next_pos, distance = next_pos - Pos)
]
...
close_snvs_nonadjacent[
  , pos_id := paste(Chrom, Pos, sep = "_")
]

close_snvs_nonadjacent[, Chrom := sub("chr", "", Chrom)] 
...
collapse_mnvs <- function(df, close_snvs) {
  for (i in seq_len(nrow(close_snvs))) {
    ...
    mut1 <- df[df$sampleID == sample & df$chr == chrom & df$pos == pos1]
    mut2 <- df[df$sampleID == sample & df$chr == chrom & df$pos == pos2]
    ...
    df <- df[!(sampleID == sample & chr == chrom & pos %in% c(pos1, pos2))]
...
# Collapse the mutations
mutations_collapsed <- collapse_mnvs(mutations, close_snvs_nonadjacent)

# View the updated mutations dataframe
head(mutations_collapsed)

# Print results
cat("Number of unique pos_ids:", num_unique_pos_ids, "\n")

# get extended context
context <- fread("data/calls/all_calls_mut_context_GRCh38.txt") %>%
...
print(paste0(nrow(final_muts) - nrow(mutations)," SNVs were collapsed into ",
             "dinucleotides"))
...
for (i in seq_along(cohorts)) {
  ...
  saveRDS(dndsout, file = paste0("outputs/", cohorts[i], "_dndsout_PDv38is.rds"))
  write.table(dndsout$sel_cv,
              paste0("outputs/", cohorts[i], "_dndsout_selcv_PDv38is.tsv"),
              sep = "\t", col.names = T, row.names = F, quote = F)
  write.table(dndsout$annotmuts, paste0("outputs/", cohorts[i], "_dndsout_annotmuts_PDv38is.tsv"), sep = "\t", col.names = T, row.names = F, quote = F)
